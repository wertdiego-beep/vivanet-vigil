// Función serverless de Vercel: /api/tecnico-actualizar-cita
// Permite a un técnico marcar una visita como "en camino" o completarla con
// un comentario y una foto (ya comprimida a base64 en el navegador). Antes de
// escribir nada, verifica que quien llama sea realmente el técnico asignado a
// esa cita (no cualquier usuario autenticado). Usa la cuenta de servicio para
...
export default async function handler(req, res) {
  ...
  const { idToken, clienteUid, citaId, accion, comentario, fotoBase64 } = req.body || {};
  if (!idToken || !clienteUid || !citaId || !accion) {
    res.status(400).json({ error: 'Faltan datos' });
    return;
  }
  if (!['en_camino', 'completar'].includes(accion)) {
  ...
  try {
    ...
    console.error('Error en actualización de cita del técnico:', err);
